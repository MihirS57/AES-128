...
def invXORRound(ct_box,key_box):
    output_box = ct_box.copy()
    for i in range(4):
        for j in range(4):
            output_box[i][j] = output_box[i][j]^key_box[i][j]
    return output_box

def invShiftRows(pt_box):
    out_box = pt_box.copy()

    #2nd Row
    p13 = out_box[1][3]
    for i in range(3,0,-1):
        out_box[1][i] = out_box[1][i-1]
    out_box[1][0] = p13
...
def mixColumns(ct_box):
    ...
    for i in range(4):
        for j in range(4):
            out_box[i][j] = gmul(ct_box[i][0], c[0][j]) ^ gmul(ct_box[i][1], c[1][j]) ^ gmul(ct_box[i][2], c[2][j]) ^ gmul(ct_box[i][3], c[3][j])
    return out_box

def invByteSub(pt_box):
    out_box = pt_box.copy()
    for i in range(4):
        for j in range(4):
            out_box[i][j] = reverse_lookup(pt_box[i][j])
    return out_box

def decryptIt(ciphertext, round_keys):
    round_output = ciphertext.copy()
    xor_output = invXORRound(ciphertext,round_keys[10])
    shift_output = invShiftRows(xor_output)
    byte_output = invByteSub(shift_output)
    xor_output = invXORRound(byte_output,round_keys[9])
    round_output = xor_output.copy()
    for i in range(8,-1,-1):
        mix_output1 = mixColumns(round_output)
        mix_output2 = mixColumns(mix_output1)
        mix_output3 = mixColumns(mix_output2)

        shift_output = invShiftRows(mix_output3)

        sub_output = invByteSub(shift_output)

        xor_output = invXORRound(sub_output,round_keys[i])
